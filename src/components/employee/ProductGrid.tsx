...
export default function ProductGrid({ locationId }: { locationId: string }) {
  ...
  if (error) return <div className="text-center py-16 text-red-500 dark:text-red-400 text-sm">Failed to load products. Please refresh.</div>
  ...
  return (
    <div>
      ...
      {vendors.length > 1 && (
        ...
        >
          <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm border-b border-slate-100 dark:border-zinc-800 px-4 py-2.5 flex gap-2 overflow-x-auto">
            {vendors.map(v => (
              <button
                key={v}
                onClick={() => scrollToVendor(v)}
                className="px-3 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 hover:bg-indigo-100 dark:hover:bg-indigo-900 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors whitespace-nowrap shrink-0"
              >
                {v}
              </button>
            ))}
        ...
        {vendors.map(vendor => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm overflow-hidden scroll-mt-[120px]"
            >
              <button
                onClick={() => toggleCollapse(vendor)}
                className="w-full flex items-center justify-between px-4 py-2.5 border-b border-slate-50 dark:border-zinc-800 bg-slate-50/60 dark:bg-zinc-800 hover:bg-slate-100/60 dark:hover:bg-zinc-800 transition-colors"
              >
                <h3 className="text-xs font-semibold text-slate-500 dark:text-zinc-400 uppercase tracking-wider">{vendor}</h3>
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] text-slate-400 dark:text-zinc-500">{vendorProducts.length}</span>
                  <ChevronDown
                    size={14}
                    className={`text-slate-400 dark:text-zinc-500 transition-transform duration-200 ${isCollapsed ? '-rotate-90' : ''}`}
                  />
                </div>
              </button>
              {!isCollapsed && (
