...
function MetaTag({ item, onDelete, onRename }: {
  ...
  if (editing) {
    return (
      <span className="flex items-center pl-2 pr-1 py-1 bg-indigo-50 dark:bg-indigo-950 border border-indigo-200 dark:border-indigo-900 rounded-xl">
        <input
          value={val}
          ...
          className="text-sm bg-transparent outline-none text-indigo-700 dark:text-indigo-300 w-24"
          autoFocus
        />
      </span>
    )
  ...
  return (
    <span
      ...
      className="flex items-center gap-1.5 pl-3 pr-2 py-1.5 bg-slate-100 dark:bg-zinc-800 rounded-xl text-sm text-slate-700 dark:text-zinc-200 cursor-default select-none"
    >
      {item.name}
      <button
        onClick={onDelete}
        className="w-4 h-4 rounded-full hover:bg-slate-300 dark:hover:bg-zinc-700 flex items-center justify-center transition-colors text-slate-400 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400"
      >
        <Trash2 size={11} />
      </button>
    </span>
...
function MetaSection({ icon, title, items, loading, onAdd, onDelete, onRename, placeholder }: MetaSectionProps) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-50 dark:border-zinc-800">
        <span className="text-slate-400 dark:text-zinc-500">{icon}</span>
        <h2 className="font-semibold text-slate-900 dark:text-zinc-100">{title}</h2>
      </div>
      <div className="p-4 border-b border-slate-50 dark:border-zinc-800">
        <div className="flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)}
            ...
            className="flex-1 px-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
      ...
      <div className="p-4">
        {loading ? <div className="flex justify-center py-4"><Spinner /></div>
          : !items?.length ? <p className="text-slate-400 dark:text-zinc-500 text-sm text-center py-2">None yet.</p>
          : (
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-start gap-2 px-2 py-2 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800 group">
      <button {...attributes} {...listeners}
        className="text-slate-300 dark:text-zinc-600 hover:text-slate-500 dark:hover:text-zinc-400 cursor-grab active:cursor-grabbing touch-none p-1 shrink-0 mt-0.5">
      ...
      <div className="flex-1 min-w-0">
        {editName ? (
          <input
            ...
            className="text-sm font-medium px-1 py-0.5 border border-indigo-300 dark:border-indigo-800 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-400 w-full"
            autoFocus
          />
        ) : (
          <span
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
          ...
          <button
            ...
            className={`flex items-center gap-1 text-xs transition-colors rounded px-1 ${hide_unit ? 'text-indigo-600 dark:text-indigo-400 font-medium' : 'text-slate-300 dark:text-zinc-600 hover:text-slate-500 dark:hover:text-zinc-400'}`}
          >
      ...
      <button
        onClick={onDelete}
        className="p-1 rounded-lg text-slate-300 dark:text-zinc-600 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950 transition-colors opacity-0 group-hover:opacity-100 shrink-0 mt-0.5"
      >
        <Trash2 size={13} />
      </button>
    </div>
...
function VendorSection() {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-50 dark:border-zinc-800">
        <span className="text-slate-400 dark:text-zinc-500"><Tag size={18} /></span>
        <h2 className="font-semibold text-slate-900 dark:text-zinc-100">Vendors</h2>
      </div>
      <div className="p-4 border-b border-slate-50 dark:border-zinc-800">
        <div className="flex gap-2">
          <input
      ...
      <div className="p-4">
        {isLoading ? <div className="flex justify-center py-4"><Spinner /></div>
          : !vendors?.length ? <p className="text-slate-400 dark:text-zinc-500 text-sm text-center py-2">None yet.</p>
          : (
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-slate-400 dark:text-zinc-500 text-sm mt-1">Manage locations, employees, and product options</p>
      </div>

      {/* Locations */}
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-50 dark:border-zinc-800">
          <div className="flex items-center gap-2"><MapPin size={18} className="text-slate-400 dark:text-zinc-500" /><h2 className="font-semibold text-slate-900 dark:text-zinc-100">Locations</h2></div>
        ...
        <div className="divide-y divide-slate-50 dark:divide-zinc-800">
            ...
            : locations?.map(loc => (
              <div key={loc.id} className="flex items-center justify-between px-5 py-3">
                <Link to={`/admin/locations/${loc.id}`}
                  className="flex items-center gap-1.5 text-sm font-medium text-slate-700 dark:text-zinc-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group flex-1 min-w-0">
                  {loc.name}
                  <ChevronRight size={14} className="text-slate-300 dark:text-zinc-600 group-hover:text-indigo-400 transition-colors shrink-0" />
                </Link>
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-300 dark:text-zinc-600 hover:text-indigo-500 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors"
                  >
                    <Link2 size={15} />
                  </button>
          ...
          {!loadingLocs && !locations?.length && <div className="px-5 py-6 text-center text-slate-400 dark:text-zinc-500 text-sm">No locations yet.</div>}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-50 dark:border-zinc-800">
          <div className="flex items-center gap-2"><Users size={18} className="text-slate-400 dark:text-zinc-500" /><h2 className="font-semibold text-slate-900 dark:text-zinc-100">Employees</h2></div>
        ...
        <div className="divide-y divide-slate-50 dark:divide-zinc-800">
          {loadingEmps ? <div className="flex justify-center py-8"><Spinner /></div>
            : employees?.map(emp => (
              <div key={emp.id} className="flex items-center justify-between px-5 py-3">
                <div>
                  <p className="text-sm font-medium text-slate-700 dark:text-zinc-200">{emp.name}</p>
          ...
          {!loadingEmps && !employees?.length && <div className="px-5 py-6 text-center text-slate-400 dark:text-zinc-500 text-sm">No employees yet.</div>}
        </div>
      </div>
      ...
      <Modal open={locModal} onClose={() => setLocModal(false)} title="Add Location">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-500 dark:text-zinc-400 mb-1.5">Location Name</label>
            <input value={locName} onChange={e => setLocName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCreateLocation()}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Downtown" autoFocus />
          </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={empForm.name} onChange={e => setEmpForm(f => ({ ...f, name: e.target.value }))}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Employee name" autoFocus />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-500 dark:text-zinc-400 mb-2">Locations</label>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {locations?.map(l => {
                const checked = empForm.location_ids.includes(l.id)
                return (
                  <label key={l.id} className={`flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer transition-colors ${checked ? 'bg-indigo-50 dark:bg-indigo-950 border border-indigo-200 dark:border-indigo-900' : 'border border-slate-100 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800'}`}>
                    <input
                      type="checkbox"
                      checked={checked}
                      ...
                      className="w-4 h-4 rounded border-slate-300 dark:border-zinc-700 text-indigo-600 dark:text-indigo-400 focus:ring-indigo-500 accent-indigo-600"
                    />
                    <span className={`text-sm font-medium ${checked ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-zinc-200'}`}>{l.name}</span>
                  </label>
                )
              })}
            </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setEmpForm(f => ({ ...f, active: !f.active }))}
              className={`relative w-10 h-6 rounded-full transition-colors ${empForm.active ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-zinc-800'}`}>
              <span className={`absolute top-1 w-4 h-4 bg-white dark:bg-zinc-900 rounded-full shadow transition-transform ${empForm.active ? 'translate-x-5' : 'translate-x-1'}`} />
            </button>
            <span className="text-sm text-slate-600 dark:text-zinc-300">Active</span>
          </div>
